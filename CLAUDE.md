...
## Claude Chat Mode (JSON Streaming)

The "claude chat" session type uses Claude CLI with `--output-format stream-json` for a web chat UI.

### Files

| File | Purpose |
|------|---------|
| `sandboxer/chat.py` | Claude subprocess management, message sync |
| `sandboxer/static/chat.js` | Fullscreen chat page JS |
| `sandboxer/templates/chat.html` | Fullscreen chat template |

### Claude JSON Event Types

When using `claude -p --output-format stream-json`, Claude sends these event types:

| Type | Subtype | Description |
|------|---------|-------------|
| `system` | `init` | Session initialized. Contains `session_id`, `model`, `tools[]` |
| `assistant` | - | Claude's response. `message.content[]` has text/tool_use blocks |
| `user` | - | Tool result. `message.content[]` has `tool_result` blocks |
| `result` | `success` | Response complete. Contains `duration_ms`, `total_cost_usd`, `usage` |
...
| `tool_use` | `id`, `name`, `input` | Tool invocation (Bash, Read, Edit, etc.) |

### Tool Result (in `user.message.content[]`)

| Type | Fields | Description |
|------|--------|-------------|
| `tool_result` | `tool_use_id`, `content`, `is_error` | Tool output |

### Example Event Sequence

```
1. system (init)     → Session started, model: claude-opus-4-5
2. assistant         → tool_use: Bash {command: "ls"}
3. user              → tool_result: "file1.txt\nfile2.txt"
4. assistant         → text: "Here are the files..."
5. result (success)  → duration: 5.2s, cost: $0.0234
```
...
| GET | `/api/chat-sync?session=NAME` | SSE endpoint for cross-tab sync |
| POST | `/api/chat-toggle` | Switch between chat/cli modes |
| GET | `/chat?session=NAME` | Fullscreen chat page |

### Session Sync

Chat sessions sync across tabs/windows in real-time:
- Messages broadcast via SSE to all connected clients
- History replayed on new connection
- Input state synced (type in one tab, see in another)
